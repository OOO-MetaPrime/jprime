...
public class JsonExpr {
  ...
  public static Filter toFilter(JsonExpr exp) {
    ...
    if (c != null) {
      ...
      if (c.getEq() != null) {
        ...
        return Filter.attr(c.getAttr()).inQuery(toSubquery(c.getInQuery()));
      } else if (c.getNotInQuery() != null) {
        return Filter.attr(c.getAttr()).notInQuery(toSubquery(c.getNotInQuery()));
  ...
  public static JsonExpr toJson(Filter filter,
    ...
    if (filter instanceof PeriodFeatureFilter v) {
    ...
    } else if (filter instanceof YearFilter v) {
      ...
      if (v.getOper() == FilterOperation.EQ) {
        ...
        cond = JsonCond.newAttrCond(attrName).inQuery(toJsonSubquery(inst.getValue()));
      } else if (v.getOper() == FilterOperation.NOT_IN_QUERY) {
        NotINQuery inst = (NotINQuery) v;
        cond = JsonCond.newAttrCond(attrName).notInQuery(toJsonSubquery(inst.getValue()));
   ...
   * @return JsonSubquery
   */
  private static JsonSubquery toJsonSubquery(JPSubQuery v) {
   ...
   * @return JPSubQuery
   */
  private static JPSubQuery toSubquery(JsonSubquery v) {
    if (v == null) {
