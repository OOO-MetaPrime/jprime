...
public class JPResourceAccessCommonService implements JPResourceAccessService {
  ...
  private Filter in(String attrCode, Collection<String> values, AuthInfo auth) {
    boolean isOktmoPattern = values.contains(FilterValue.AUTH_OKTMO);
    boolean isOktmoTreePattern = values.contains(FilterValue.AUTH_OKTMO_TREE);
    boolean isSubjectGroupPattern = values.contains(FilterValue.AUTH_SUBJECT_GROUP);
