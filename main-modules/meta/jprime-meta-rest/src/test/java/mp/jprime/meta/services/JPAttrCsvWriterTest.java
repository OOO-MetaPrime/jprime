...
import mp.jprime.parsers.services.ParserBaseService;
...
@SpringBootTest(classes = {JPAttrCsvWriterBaseService.class, JPClassJsonBaseConverter.class, JPJsonMapper.class,
    ParserBaseService.class, ParserServiceAwareConfiguration.class,
...
public class JPAttrCsvWriterTest {
  private static final JPClass TEST_VALUE = JPClassBean.newBuilder().attrs(new ArrayList<>() {{
        ...
        this.add(JPAttrBean.newBuilder().code("code4").name("name4").schemaProps(List.of(JPPropertyBean.builder().code("prop1Code").type(PropertyType.INT).build())).build());
  ...
  @Test
  void shouldWrite() {
    String lineSplit = System.lineSeparator();
