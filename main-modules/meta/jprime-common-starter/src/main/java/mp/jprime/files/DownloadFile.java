...
public interface DownloadFile {
  ...
  default Mono<Void> writeTo(ServerWebExchange swe, JPFileThumbnail thumbnail, String userAgent) {
    return writeTo(swe, thumbnail.isOriginal() ? HttpStatus.OK : HttpStatus.NO_CONTENT,
        thumbnail.getInputStream(), thumbnail.getFileName(), null,
        FileTypeDetector.mediaType(thumbnail.getFileName()), userAgent);
  }

  default Mono<Void> writeTo(ServerWebExchange swe, InputStream is, String fileTitle, String userAgent) {
    return writeTo(swe, is, fileTitle, null, FileTypeDetector.mediaType(fileTitle), userAgent);
  }

  default Mono<Void> writeTo(ServerWebExchange swe, InputStream is, String fileTitle, Long fileLength, String userAgent) {
    return writeTo(swe, is, fileTitle, fileLength, FileTypeDetector.mediaType(fileTitle), userAgent);
  }
  ...
  default Mono<Void> writeTo(ServerWebExchange swe, InputStream is, String fileTitle, Long fileLength,
    ...
    return writeTo(swe, HttpStatus.OK, is, fileTitle, fileLength, contentType, userAgent);
  }

  default Mono<Void> writeTo(ServerWebExchange swe, HttpStatus status,
                             InputStream is, String fileTitle, Long fileLength,
                             String contentType, String userAgent) {
