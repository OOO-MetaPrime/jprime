...
/**
 * Реализация JPAccessService
 */
@Service
public class JPAccessCommonService implements JPAccessService {
  ...
  private boolean prePIP(Policy policy, AuthInfo auth) {
    ...
    for (EnviromentRule enviromentRule : policy.getEnviromentRules()) {
      if (!check(enviromentRule, auth)) {
        return false;
      }
    }
    return true;
  ...
  private boolean check(EnviromentRule rule, AuthInfo auth) {
    if (rule == null) {
      return Boolean.TRUE;
    }
