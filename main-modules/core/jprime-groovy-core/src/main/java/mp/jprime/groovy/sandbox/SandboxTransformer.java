package mp.jprime.groovy.sandbox;

import groovy.lang.Script;
import mp.jprime.groovy.sandbox.impl.Checker;
import mp.jprime.groovy.sandbox.impl.Ops;
import mp.jprime.groovy.sandbox.impl.SandboxedMethodClosure;
  ...
  class VisitorImpl extends ScopeTrackingClassCodeExpressionTransformer {
    ...
    private Expression innerTransform(Expression exp) {
        ...
        try (StackVariableSet scope = new StackVariableSet(this)) {
          ...
          if (parameters != null) {
            ...
            if (parameters.length > 0) {
              ...
              declareVariable(new Parameter(ClassHelper.OBJECT_TYPE, "it"));
            }
          }
      ...
      if (exp instanceof BinaryExpression) {
        ...
        } else if (be.getOperation().getType() == Types.KEYWORD_INSTANCEOF) {// instanceof operator
          return super.transform(exp);
        } else if (Ops.isLogicalOperator(be.getOperation().getType())) {
          return super.transform(exp);
        } else if (be.getOperation().getType() == Types.KEYWORD_IN) {// membership operator: JENKINS-28154
