...
Количество топиков в каскаде и соответствующие интервалы задержки при ошибке обработки сообщения из топиков каскада задаются имплементацией
метода `protected abstract Collection<Long> getCascadeIntervals()`. Если метод вернет пустую коллекцию, то будет создан
только один слушатель основного топика с `сascadeIntervals = 0`.

При обработке событий в пакетном режиме `getMaxPollRecords() > 1` важно генереировать `BatchListenerFailedException` с указанием сообщения или его индекса в пакете для компенсации именно ошибочного сообщения.
...
class MyClass extends JPKafkaDeadLetterConsumerService<String, String> {
  ...
  private List<Long> сascadeIntervals;
