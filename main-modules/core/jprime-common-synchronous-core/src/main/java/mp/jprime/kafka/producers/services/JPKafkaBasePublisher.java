...
public abstract class JPKafkaBasePublisher<E, K, V> implements JPKafkaPublisher<E> {
  ...
  @Override
  public void publishAsync(Collection<E> events, Consumer<Throwable> failureCallback) {
    if (CollectionUtils.isEmpty(events)) {
      return;
    }
  ...
  private void doSendAsync(Collection<ProducerRecord<K, V>> events, Consumer<Throwable> failureCallback) {
    KafkaOperations<K, V> kafkaOperations = getKafkaOperations();
    events.forEach(record -> kafkaOperations
        .send(record)
        .whenCompleteAsync((result, e) -> {
