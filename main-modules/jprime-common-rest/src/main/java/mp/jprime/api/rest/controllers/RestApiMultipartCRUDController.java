...
public class RestApiMultipartCRUDController extends DownloadFileRestController implements JsonMapper {
  ...
  public Mono<ResponseEntity> downloadFile(ServerWebExchange swe,
    ...
    return Mono.just(metaStorage.getJPClassByPluralCode(pluralCode))
        ...
        .switchIfEmpty(Mono.error(new ResponseStatusException(HttpStatus.NOT_FOUND)))
        .flatMap(
            fileInfo -> read(fileInfo, userAgent)
        );
  }
        ...
        .flatMap(jpClass -> {
              ...
              return Mono.zip(
                  Flux.from(cache)
                      ...
                      .flatMap(
                          x -> x.content()
                              .collect(() -> new UploadInputStream(x.name()), (t, dataBuffer) -> t.collectInputStream(dataBuffer.asInputStream()))
                      )
                      .map(x -> {
